              ...
              <div style={{ overflowX:'auto' }}>
                <table style={{ borderCollapse:'collapse', tableLayout:'fixed', width:'auto', minWidth: isMobile ? 'unset' : '100%' }}>
                  <colgroup>
                    ...
                    {activeMassCols.map(sz => <col key={sz} style={{ width: isMobile ? 50 : 70 }} />)}
                  </colgroup>
                  <thead>
                    <tr style={{ background:'#f8f7f4' }}>
                      ...
                      {activeMassCols.map(sz => (
                        <th key={sz} style={{ padding: isMobile ? '5px 4px' : '6px 8px', textAlign:'center', fontSize:10, color:'#64748b', fontWeight:700, borderBottom:'2px solid #e2e8f0' }}>{MASS_LABELS[sz]}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {sortedMassKats.map(kat => (
                      <React.Fragment key={kat}>
                        <tr>
                          <td colSpan={activeMassCols.length + 1} style={{ background: KAT_COLOR[kat]||'#f1f5f9', padding:'5px 8px', fontWeight:700, fontSize:11, color:'#374151', borderTop:'2px solid #e2e8f0' }}>
                            {kat}
                          </td>
                        </tr>
                              ...
                              return (
                                <td key={sz} style={{ padding: isMobile ? '3px 2px' : '3px 4px', textAlign:'center' }}>
                                  <input
                                    ...
                                    placeholder="0"
                                    style={{ width:'100%', padding: isMobile ? '6px 2px' : '5px 4px', textAlign:'center', border: `2px solid ${hasVal ? '#10b981' : '#e2e8f0'}`, borderRadius:6, fontSize: isMobile ? 14 : 13, fontWeight:700, outline:'none', boxSizing:'border-box', background: hasVal ? '#f0fdf4' : '#fff', color:'#111', colorScheme:'light' }}
                                  />
                                  <div style={{ fontSize:9, color:'#94a3b8', marginTop:1 }}>s:{saldo}</div>
                                </td>
                              );
                            })}
                          </tr>
